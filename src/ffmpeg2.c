...
void encode(ffmpeg_context* ctx, int skip);
...
void init_context(ffmpeg_context* ctx, unsigned int width, unsigned int height) {
    ...
    ctx->codec = avcodec_find_encoder(AV_CODEC_ID_VP9);
...
int main(int argc, char** argv) {
    ...
    //     int err = avio_close(ctx.formatContext->pb);
    //     if (err < 0) {
